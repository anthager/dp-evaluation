...
METRICS = ['dataset_size', 'time_rmspe']
...
path = utils.SQL_QUERIES_PATH + 'extractor-time-dataset_size.sql'
...
def run():
    ...
    plt.figure()
    for dv in DIMENSION_VALUES:
        plt.plot(results[dv]['dataset_size'],
                 results[dv]['time_rmspe'], label=dv)
        plt.xlabel('Dataset Size')
        plt.ylabel('Time RMSPE')
        plt.legend()
        plt.savefig(utils.RESULTS_PATH + '/' + 'plot' + '-' + 'time-dataset_size' + '-' + dv + '.png', dpi=200,
